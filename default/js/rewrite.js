function rewrite_add()
{
	...
	var host = $("#host").val();
	if (host == "") {
		return layer.alert('域名不能为空',{icon:'error'});
	}
	if (dst == "") {
		return layer.alert('跳转域名不能为空',{icon:'error'});
	}
	if (code == "") {
		code = 301;
	}
...
function example()
{
	document.getElementById('host').value = 'www.nanyinet.com';
	document.getElementById('dst').value = 'nanyinet.com';
	close_msg();
}
function close_msg()
{
